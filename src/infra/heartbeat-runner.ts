...
export async function runHeartbeatOnce(opts: {
  ...
  // EXCEPTION: Don't skip for exec events or hook wakes - they have pending system events to process.
  const isExecEventReason = opts.reason === "exec-event";
  const isHookWakeReason = typeof opts.reason === "string" && opts.reason.startsWith("hook:");
  const isEventTriggered = isExecEventReason || isHookWakeReason;
